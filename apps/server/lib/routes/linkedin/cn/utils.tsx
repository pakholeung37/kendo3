    ...
    return cache.tryGet(`linkedincn:${entityUrn}`, async () => {
        ...
        return {
            ...
            description: renderToString(
                <>
                    <h2>{job.title}</h2>
                    <p>{job.applyMethod?.instantOffsiteApply ? <a href={job.applyMethod.instantOffsiteApply.companyApplyUrl}>点击申请</a> : null}</p>
                    <p>{job.applyMethod?.basicOffsiteApply ? <a href={job.applyMethod.basicOffsiteApply.companyApplyUrl}>点击申请</a> : null}</p>
                    <p>
                        已有{job.numApplies}人申请此职位， {job.numViews}人查看此职位
                    </p>
                    {job.compensationDescription ? <p>薪资：{job.compensationDescription || 'N/A'} </p> : null}
                    <p>工作地点： {job.geo?.defaultLocalizedName ?? ''} </p>
                    {job.company ? (
                        <>
                            <h2>公司介绍</h2>
                            <p>{job.company.name}</p>
                            <p>员工人数：{job.company.employeeCount}</p>
                            <p>{job.company.localizedDescription}</p>
                        </>
                    ) : null}
                    <h2>职位介绍</h2>
                    <article>{job.desc ? raw(job.desc) : null}</article>
                </>,
            ),
